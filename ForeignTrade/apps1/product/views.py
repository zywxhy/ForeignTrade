        ...
        # 收到ztree类以及检索信息，则返回检索到的queryset类
            ...
            if user.language == 'Spanish':
                ...
                product = Product.objects.filter(Q(product_id__contains=search) | Q(name__contains=search) |
                                                 Q(spec__contains=search)|Q(type__type_name=search) |
                                                 Q(model__contains=search)).order_by('product_id')
    ...
    def get(self,request):
        ...
        if search is not None:
            if request.user.language == 'Spanish':
                ...
                product = Product.objects.filter(Q(product_id__contains=search) | Q(name__contains=search) |
                                                 Q(spec__contains=search) | Q(model__contains=search)
                                                 |Q(type__type_name=search)).order_by(
                    'product_id')
        else:
