        ...
        if search is not None:
            if request.user.language == 'Spanish':
                product = Product.objects.filter(Q(product_id__contains=search) | Q(spanish_name__contains=search) |
                                                 Q(spanish_spec__contains=search) | Q(peru_name__contains=search) |
                                                 Q(peru_spec__contains=search) | Q(mexico_name__contains=search) |
                                                 Q(mexico_spec__contains=search) | Q(type__type_name=search)|Q(model__icontains=search))\
                                                 .order_by('product_id')
            else:
                product = Product.objects.filter(Q(product_id__contains=search) | Q(name__contains=search) |Q(spec__contains=search) | Q(type__type_name=search)
                                                 | Q(model__icontains=search)).order_by('product_id')
        else:
