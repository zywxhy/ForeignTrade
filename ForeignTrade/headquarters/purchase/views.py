...
class PurchaseAddView(View):
    ...
    def post(self,request):
        ...
        product_split = DataSplit(request.POST.get('purchase_product'),['product_id', 'remark', 'purchase_count', 'unit_price', 'amount'], PurchaseProduct)
...
def purchase_review(request):
    ...
    condition = {'purchase_num': purchase_num}
    purchase = get_object_or_404(PurchaseContract, **condition)
    if result == 'Y':
        purchase.status = 1
        purchase.save()
        for product in purchase.purchaseproduct_set.all():
            sales_product = SalesProduct.objects.get(sales__sales_num=purchase.sales_num,product_id = product.product_id)
            sales_product.purchase_count += product.purchase_count
            sales_product.save()
...
class CustomsClearance(View):
    def post(self,request):
        clearance_list = request.POST.get('clearance_list','')
        purchase_num = request.POST.get('purchase_num','')
        if clearance_list:
            data = json.loads(clearance_list)
            for product in data:
                add_cost = product['add_cost']
