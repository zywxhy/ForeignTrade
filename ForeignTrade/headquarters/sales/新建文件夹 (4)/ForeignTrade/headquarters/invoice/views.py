...
class InvoiceAddView(View):
    def get(self,request):
        ...
        fields = ['product__model','product__name','product__spec','product__product_id','remark','outbound_count','sales_count']
        layui = Layui(request,product_list,fields,len(product_list),)
        products = layui.laytable_data(is_json=True)
...
class InvoiceModifyView(View):
    def get(self,request):
        ...
        product_list = SalesProduct.objects.filter(sales_num=invoice.sales.sales_num, sales_count__gt=F('outbound_count'))
    ...
    def post(self,request):
        ...
        product_split = DataSplit(request.POST['invoice_num'],['product_id', 'remark', 'sales_count', 'unit_price', 'amount'], InvoiceProduct)
        contract_operations = ContractOperations(Invoice, form, sub_split1=product_split)
